...
if (isset($_GET['origin']) && $_GET['origin'] == "postdata") {
	if (isset($_GET['q']) && $_GET['q'] != "") {
		...
		$c = $_GET['c'];
		if ($a == "CAT") {
			$sql_adCat = "SELECT * FROM ad_category WHERE cat_list='".$c."' AND cat_name LIKE '%" . $q . "%' ORDER BY id_cat ASC";
		...
		if ($a == "CAT") {
			...
			if ($adCat->num_rows > 0) {
				echo '<div style="padding: 5px; background-color: #fff; border: 2px solid orange;" id="i_category">';
				while ($row_adCat = $adCat->fetch_assoc()) {
					?>
						<a style="cursor: pointer; display: block; padding: 5px 0;" onclick="makeCat('<?php echo $row_adCat['cat_name']; ?>')"><?php echo $row_adCat["cat_name"]; ?></a>
					<?php
				}
				echo '</div>';
			} else {
				echo '<div style="padding: 5px; background-color: #fff; border: 2px solid orange;" id="i_category">Invalid Category</div>';
			}
	...
	<?php //Personal Profile Settings Display	
		...
		<div class="msg-box row">
			...
			<form method="post" name="user_update" id="user_update"
				...
				<div class="mini">
					<input type="text" name="user_city" id="user_city" value="<?php echo $row_user['user_city']; ?>"
						placeholder="Your City" required>
					<input type="text" name="user_state" id="user_state" value="<?php echo $row_user['user_state']; ?>"
	...
	<?php //Business Profile Settings Display	
		...
		<div class="msg-box row">
			...
			<form method="post" name="biz_data" id="biz_data" action="<?php echo htmlspecialchars('inc/action_page.php'); ?>"
				...
				<input type="text" name="biz_cat" id="biz_cat" value="<?php echo $row_bizData['biz_cat']; ?>" onkeyup="showData(this.value)" placeholder="Item Category">
